...
def detect_market_state(df):
    ...
    atr_mean = atr.tail(30).mean()  # 近30根K线ATR均值
    curr_atr = atr.iloc[-1]
    # 阈值设为均值的1.2倍
    atr_threshold = 1.2 * atr_mean
    # 日志输出
    logger.debug(f"ATR均值(30): {atr_mean:.4f}, 当前ATR: {curr_atr:.4f}, 阈值: {atr_threshold:.4f}")
    # 判断市场状态
...
def main():
    ...
    logger.info("-------------------------")
    # Telegram推送（根据市场状态动态调整推送阈值）
    if output['long_score'] >= push_threshold or output['short_score'] >= push_threshold:
        # 时间调整为GMT+8
        ...
        logger.debug("分值未达到推送阈值（long_score=%d, short_score=%d, 阈值=%d），不发送Telegram消息。", output['long_score'], output['short_score'], push_threshold)
